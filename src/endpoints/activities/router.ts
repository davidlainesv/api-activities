...
/**
 * @api {get} /activities Obtiene la lista de actividades
...
router.put('/:id', async function (req, res) {
    try {
        await controller.select_activity(req.params.id);
    } catch (error) {
...
router.delete('/:id', async function (req, res) {
    try {
        await controller.select_activity(req.params.id as string);
    } catch (error) {
        return res.status(404).json({
            message: "No hemos encontrado la actividad.",
            server_error: error
